...
const ModernExperience: React.FC = () => {
  ...
  // Experience data
  const { experiences, stats, currentExperiences, pastExperiences } = useExperienceData()
  ...
  const filteredExperiences = filter === 'current' 
    ? currentExperiences 
    : filter === 'past' 
    ? pastExperiences 
    : experiences
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 mb-6">
            ...
            <span className="text-green-500 font-semibold">Professional Experience</span>
          </div>

          <h2 className={`text-4xl md:text-5xl font-bold mb-6 ${
            isDark ? "text-white" : "text-slate-900"
          }`}>
            Career <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-emerald-500">Journey</span>
          </h2>

          <p className={`text-xl max-w-3xl mx-auto leading-relaxed ${
            isDark ? "text-slate-300" : "text-slate-700"
          }`}>
            A comprehensive overview of my {stats.totalYears}+ years in web development, 
            spanning {stats.totalProjects}+ projects and {stats.totalClients}+ satisfied clients.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          ...
          <div className="flex gap-2">
            {[
              ...
              { key: 'current', label: 'Current Roles', count: currentExperiences.length },
              { key: 'past', label: 'Past Roles', count: pastExperiences.length },
            ].map(({ key, label, count }) => (
              <button
                key={key}
